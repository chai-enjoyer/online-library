  ...
  try {
    ...
    booksTable.innerHTML = books.length > 0 ? books.map(book => `
      <tr>
        ...
        <td>${book.isbn || 'N/A'}</td>
        <td>
          <button class="btn btn-warning btn-sm me-2" onclick="editBook('${book._id}', '${book.title}', '${book.authors.join(',')}', '${book.categories.join(',')}', '${book.isbn}', '${book.longDescription || ''}', '${book.pageCount}', '${book.publishedDate || ''}', '${book.shortDescription || ''}', '${book.thumbnailUrl || ''}')"><i class="fas fa-edit"></i> Edit</button>
          <button class="btn btn-danger btn-sm" onclick="deleteBook('${book._id}')"><i class="fas fa-trash"></i> Delete</button>
        </td>
      </tr>
...
async function editBook(id, currentTitle, currentAuthors, currentCategories, currentIsbn, currentLongDescription, currentPageCount, currentPublishedDate, currentShortDescription, currentThumbnailUrl) {
  ...
  const isbn = prompt('Enter new ISBN:', currentIsbn) || currentIsbn;
  ...
  try {
    const res = await fetch(`/books/${id}`, {
      ...
      body: JSON.stringify({ title, authors, categories, isbn, longDescription, pageCount, publishedDate, shortDescription, thumbnailUrl }),
    });
    if (res.ok) {
      window.location.reload();
    } else {
